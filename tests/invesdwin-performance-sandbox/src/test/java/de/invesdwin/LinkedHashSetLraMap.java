...
@NotThreadSafe
public class LinkedHashSetLraMap<K, V> implements Map<K, V> {

    private final int maximumSize;
    private final SortedSet<K> orderedKeys;
    private final Map<K, V> map = new Object2ObjectOpenHashMap<>();

    public LinkedHashSetLraMap(final int maximumSize) {
        this.maximumSize = maximumSize;
        this.orderedKeys = new ObjectLinkedOpenHashSet<>();
    }
    ...
    public V put(final K arg0, final V arg1) {
        ...
        if (put == null) {
            ...
            while (orderedKeys.size() > maximumSize) {
                map.remove(orderedKeys.remove(arg0));
            }
        }
        return put;
    }
