...
def callback(methodName, *parameters)
    if($jrubyScriptTaskCallbackContext.nil?)
        if($binding.containsKey("jrubyScriptTaskCallbackContextUuid"))
        	$jrubyScriptTaskCallbackContext = Java::DeInvesdwinContextRubyJrubyCallback::JrubyScriptTaskCallbackContext.getContext($binding.get("jrubyScriptTaskCallbackContextUuid"))
        else
