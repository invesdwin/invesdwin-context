...
@Immutable
@Named
public final class ProvidedScriptTaskRunnerFrege
        implements IScriptTaskRunnerFrege, FactoryBean<ProvidedScriptTaskRunnerFrege> {

    public static final String PROVIDED_INSTANCE_KEY = IScriptTaskRunnerFrege.class.getName();

    public static final ProvidedScriptTaskRunnerFrege INSTANCE = new ProvidedScriptTaskRunnerFrege();

    @GuardedBy("this.class")
    private static IScriptTaskRunnerFrege providedInstance;

    private ProvidedScriptTaskRunnerFrege() {}

    public static synchronized IScriptTaskRunnerFrege getProvidedInstance() {
        if (providedInstance == null) {
            ...
            if (systemProperties.containsValue(PROVIDED_INSTANCE_KEY)) {
                try {
                    ...
                    return (IScriptTaskRunnerFrege) Reflections.classForName(runner).newInstance();
                } catch (InstantiationException | IllegalAccessException e) {
                    throw new RuntimeException(e);
                }
            } else {
                final Map<String, IScriptTaskRunnerFrege> runners = new LinkedHashMap<String, IScriptTaskRunnerFrege>();
                for (final IScriptTaskRunnerFrege runner : ServiceLoader.load(IScriptTaskRunnerFrege.class)) {
                    final IScriptTaskRunnerFrege existing = runners.put(runner.getClass().getName(), runner);
                    if (existing != null) {
    ...
    public static synchronized void setProvidedInstance(final IScriptTaskRunnerFrege providedInstance) {
        ProvidedScriptTaskRunnerFrege.providedInstance = providedInstance;
        final SystemProperties systemProperties = new SystemProperties();
        if (providedInstance == null) {
    ...
    public Class<?> getObjectType() {
        return ProvidedScriptTaskRunnerFrege.class;
    }
    ...
    @Override
    public ProvidedScriptTaskRunnerFrege getObject() throws Exception {
        return INSTANCE;
    }
